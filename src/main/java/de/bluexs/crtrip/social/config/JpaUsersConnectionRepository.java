...
import de.bluexs.crtrip.service.UserService;
import de.bluexs.crtrip.social.persistence.Role;
...
public class JpaUsersConnectionRepository implements UsersConnectionRepository {
	...
	public List<String> findUserIdsWithConnection(Connection<?> connection) {
        ...
        String userId;
        if(user == null) {
          userId = null; //userService.createUser(Role.authenticated).getUserId();
        } else {
	...
	public ConnectionRepository createConnectionRepository(String userId) {
        ...
        User user = null; //userRepository.findByUuid(userId);
        if(user == null) {
            throw new IllegalArgumentException("User not Found");
        }
		return new JpaConnectionRepository(socialUserRepository, user, connectionFactoryLocator, textEncryptor);
	}
